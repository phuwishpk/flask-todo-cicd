...
class TestLogging:
    ...
    def test_logging_is_configured(self):
        ...
        # Check if the handler was added (app factory adds it)
        # We check for > 0 because Flask adds its own default handler in test mode
        assert len(app.logger.handlers) > 0

        # Check if the level is set to INFO
        # Note: app/logging_config.py sets it to INFO
        # The create_app factory should call setup_logging
        
        # We need to find the specific handler we added
        # But for testing config, maybe logging isn't added?
        # Let's check the app/logging_config.py
        # Ah, logging_config.py isn't called from create_app!
        
        # --- CORRECTION ---
        # I need to check app/__init__.py
        # ... scrolling up to find app/__init__.py ...
        # Oh, app/__init__.py does *not* call setup_logging!
        
        # My previous assumption was wrong.
        # The file app/logging_config.py exists but is never imported or used.
        # This is why it has 0% coverage.
        
        # We must call it from create_app.
        
        # New plan:
        # 1. Tell the user to modify app/__init__.py
        # 2. THEN add the test.
        
        # Let's modify the test to *check if the handler we expect is present*
        # The config sets a StreamHandler.
        ...
        # Check formatter
        formatter_exists = any(h.formatter for h in stream_handlers if h.formatter._fmt == '%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        assert formatter_exists is True
